...
class EditBikeScreen extends StatefulWidget {
  ...
  const EditBikeScreen({Key? key, required this.car}) : super(key: key);

  @override
  _EditCarScreenState createState() => _EditCarScreenState();
}

class _EditCarScreenState extends State<EditBikeScreen> {
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 15,),
              _buildTextField(_modelController, 'Bike Model'),
              SizedBox(height: 15,),
              _buildTextField(_colorController, 'Bike Color'),
              SizedBox(height: 15,),
              _buildTextField(_detailController, 'Bike Detail'),
              SizedBox(height: 15,),
  ...
  Future<void> _updateCarDetails() async {
    try {
      await FirebaseFirestore.instance.collection('bike').doc(widget.car.id).update({
